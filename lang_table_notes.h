...
ES2015 (ES6)
	...
	Classes					(classes instead of constructor functions, e.g. class Person {...}
		Derived classes		(e.g. class Employee extends Person {...}
...
C++11 (major)
    ...
    default'ed member functions (SomeType() = default; // Request compiler generated default constructor)
    delete'ed member functions  (NonCopyable(const NonCopyable&) = delete; // Forbid copy construction)
	...
	Lambdas                     (Function def. on the fly e.g. [](){...})
    ...
    std::auto_ptr is deprecated (use std::unique_ptr)
